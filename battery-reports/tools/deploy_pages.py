#!/usr/bin/env python3
"""dashboard.html -> docs/battery/index.html (GitHub Pages 배포용 복사).

사용법:
  python3 tools/build_dashboard_data.py     # 데이터 갱신
  python3 tools/assemble_dashboard.py       # dashboard.html 생성
  python3 tools/deploy_pages.py             # docs/battery/index.html 로 복사
  git add docs && git commit && git push    # Pages 자동 반영

경로를 docs/battery/ 하위로 둔 이유: 같은 저장소의 기존 Pages 사이트(docs/index.html)와
충돌하지 않고 공존하기 위해서다. 배포 주소는 <pages-url>/battery/ 가 된다.

주의: docs/battery/index.html 은 **생성물**이다. 직접 편집하지 말 것 —
...
REPO = os.path.dirname(ROOT)                      # 저장소 루트 (docs/ 는 여기 둔다)
SRC = os.path.join(ROOT, "projects", "dashboard", "dashboard.html")
DST_DIR = os.path.join(REPO, "docs", "battery")
...
nojekyll = os.path.join(REPO, "docs", ".nojekyll")
if not os.path.exists(nojekyll):
    open(nojekyll, "w").close()

print(f"배포 파일 갱신: docs/battery/index.html ({os.path.getsize(DST)//1024}KB)")
